...
function Read_gpx( Files ){
	...
	if( Points = Buf.match( /<trkpt[\S\s]*?<\/trkpt>/g )){
		Points.forEach( function( Point ){
			...
			// 追加項目
			if( Point.match( /<distance>([\d\.]+)/ )){
				Log.ExtDistance[ Cnt ] = RegExp.$1;
			}
			if( Point.match( /<gradient>(-?[\d\.]+)/ )){
				Log.ExtGradient[ Cnt ] = RegExp.$1;
			}
			if( Point.match( /<ns3:atemp>(-?[\d\.]+)/ )){
				Log.ExtTemperature[ Cnt ] = RegExp.$1;
			}
			if( Point.match( /<ns3:cad>([\d\.]+)/ )){
				Log.ExtCadence[ Cnt ] = RegExp.$1;
			}
			if( Point.match( /<ns3:hr>([\d\.]+)/ )){
				Log.ExtHeartrate[ Cnt ] = RegExp.$1;
			}
			if( Point.match( /<ns3:power>([\d\.]+)/ )){
				Log.ExtPower[ Cnt ] = RegExp.$1;
			}
